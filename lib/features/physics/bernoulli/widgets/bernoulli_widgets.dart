...
/// The diagram is always drawn on a light paper surface.
...
/// White card that hosts the figure.
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        border: Border.all(color: BernoulliColors.hairline),
        boxShadow: const [
          BoxShadow(
...
class InfoPanel extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      decoration: BoxDecoration(
        color: background ?? BernoulliColors.blueSoft,
        borderRadius: BorderRadius.circular(18),
        border: Border.all(color: border ?? BernoulliColors.accent.withValues(alpha: 0.25)),
      ),
      child: child,
    );
  }
...
/// Each station's bar splits into static pressure and pressure of motion.
    ...
    return Semantics(
      ...
      child: ExcludeSemantics(
        child: Column(
          ...
          children: [
            const Text('p + ½ρv²  is the same at every station',
                style: TextStyle(
            ...
            const Text(
              ...
              style: TextStyle(fontSize: 13, color: BernoulliColors.inkSoft),
            ),
            ...
            Wrap(
              ...
              runSpacing: 6,
              children: [
                _key(BernoulliColors.accent, 'static pressure p'),
                _key(BernoulliColors.motion, 'motion ½ρv²'),
              ],
            ),
          ],
        ),
          ...
          children: [
            Text('${(total / 1000).toStringAsFixed(1)} kPa total',
                style: const TextStyle(
                    fontSize: 11, color: BernoulliColors.inkSoft)),
            const SizedBox(height: 4),
            AnimatedContainer(
            ...
            AnimatedContainer(
              ...
              height: staticH,
              decoration: BoxDecoration(
                color: negative ? BernoulliColors.bad : BernoulliColors.accent,
                borderRadius:
            ...
            Text(name,
                style: TextStyle(
                    fontWeight: FontWeight.bold,
                    color: narrow ? BernoulliColors.bad : BernoulliColors.ok)),
            if (negative)
              const Text('below zero',
                  style: TextStyle(fontSize: 11, color: BernoulliColors.bad)),
          ],
        ),
      ),
    );
...
class Readout extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final tint = color ?? BernoulliColors.accent;
    return Semantics(
      label: '$label: $value',
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          color: BernoulliColors.surface,
          borderRadius: BorderRadius.circular(12),
          border: Border.all(color: tint.withValues(alpha: 0.35)),
          boxShadow: const [
            BoxShadow(
              color: Color(0x0A203A5C),
              blurRadius: 10,
              offset: Offset(0, 4),
            ),
          ],
        ...
        child: Column(
          ...
          children: [
            Text(label,
                style: const TextStyle(
                    fontSize: 12, color: BernoulliColors.inkSoft)),
